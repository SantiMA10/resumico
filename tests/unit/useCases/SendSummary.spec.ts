...
describe('SendSummary', () => {
	...
	beforeEach(() => {
		...
		subject = new SendSummary(messageService, speechToTextService, summaryService);
	});
	...
	it('reacts with a green check if the message has been summarize', async () => {
		jest.spyOn(messageService, 'downloadAudio').mockResolvedValue({ filePath: './file.ogg' });
		jest
	...
	it('does not send the transcription to the user', async () => {
		jest.spyOn(messageService, 'downloadAudio').mockResolvedValue({ filePath: './file.ogg' });
		jest
			.spyOn(speechToTextService, 'transcribe')
			.mockResolvedValue({ text: 'hola', duration: 123 });
		...
		expect(messageService.sendMessage).not.toHaveBeenNthCalledWith(
			1,
			'user',
	...
	it('reacts with a red cross if the summary is empty', async () => {
		jest.spyOn(messageService, 'downloadAudio').mockResolvedValue({ filePath: './file.ogg' });
		jest
			.spyOn(speechToTextService, 'transcribe')
			.mockResolvedValue({ text: 'hola', duration: 123 });
